...
aiRoutes.post(
  ...
  describeRoute({
    ...
    summary: 'RAG 問答',
    description:
      '使用自然語言詢問攀岩相關問題，系統根據平台資料生成回答（需登入，受等級配額限制）。加上 `?stream=true` 可啟用 SSE 串流回應（Content-Type: text/event-stream），逐詞推送 `{"type":"token","token":"..."}` 事件，工具執行時推送 `{"type":"progress","tool":"...","status":"executing"|"done"}` 事件，結束時推送 `{"type":"done",...}` 事件。',
    responses: {
      200: {
